...
public class GaudService 
{
	...
	private static String TYPE_AREA ="1";
	private static String TYPE_GIRD ="2";
	private static String TYPE_SIZE ="3";
	
	private int test = 0;
	
	private static Map<String,GaudTypeBean> XY_MAP = new HashMap<String,GaudTypeBean>();
	...
	public int saveGuadData(String data,int taskFlag)
	{
		TYPE_MAP.clear();
		 
		p = CfgTools.readCfgFile("sys.cfg");
		...
		try
        {
		    area = new String(p.getProperty("area").getBytes("ISO-8859-1"),"utf8");
		    cityname = new String(p.getProperty("cityname").getBytes("ISO-8859-1"),"utf8");
        }
		...
		int total = 0;
		System.out.println("FIRST_PATH:"+FIRST_PATH);
		if (p.get("type").equals(TYPE_AREA))
		{
		    System.out.println("FIRST_PATH1111:"+FIRST_PATH);
		    
		    total = saveGuadDataByCity(data,taskFlag);
		}
		else
		{
		    total = saveGuadDataBySize(data,taskFlag);
		}
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	public int saveGuadDataBySize(String data,int taskFlag)
    {
        ...
        String fileName = createFileByRule();
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	private String createFileByRule()
	{
		 String fileName ="";
		
		 if (FILENAME.equals(""))
		 {
			 fileName = FIRST_PATH +"guadData";
			 String timeStamp = TimeTools.getDataByFormat("yyyyMMddHHmmss");
			 fileName =  fileName +"_"+ timeStamp +"_"+ (index++) + ".csv";
		 ...
		 else
		 {
			  ...
			  if (!fileTemp.exists()||fileTemp.length() >= 50*1024*1024)
			  {
				  fileName = FIRST_PATH +"guadData";
					 String timeStamp = TimeTools.getDataByFormat("yyyyMMddHHmmss");
					 fileName =  fileName +"_"+ timeStamp +"_"+ (index++) + ".csv";
    ...
    private String createFileByAreaRule()
    {
         ...
         if (FILENAME.equals(""))
         {
             fileName = FIRST_PATH + area+ "//" + cityname + "//"+"guadData";
             String timeStamp = TimeTools.getDataByFormat("yyyyMMddHHmmss");
             fileName =  fileName +"_"+ timeStamp +"_"+ (index++) + ".csv";
    ...
    public int saveGuadDataByCity(String data,int taskFlag)
    {
        ...
        try 
        {
            ...
            for(int i = 0; i < array.size(); i++)
            {
                ...
                if (!cityCode.equals(p.get("citycode")))
                {
                    continue;
                }
