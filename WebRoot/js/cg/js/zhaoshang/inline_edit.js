...
$('span[ectype="inline_edit"]').live("click",function(){
	...
	var target_html =  $($(this).attr("datatarget")).html();
	var $this = $(this);
	if (type=="select") {
		var selectedid = $(this).attr("selectedid");
		$(target_html).appendTo($(this).parent())
		.css({border:"1px solid #ccc",outline:"none",height:"20px;"})
		.focus()
		.change(function(){
			$(this).prev('span').show().text($(this).find("option[value="+$(this).val()+"]").text());
			if (!$(this).val()) {
				$('span[fieldname="'+s_name+'"][fieldid="'+s_id+'"]').text(s_value);
			}else {
				var new_val = $(this).val();
				$.get(url,{id:s_id,column:s_name,value:new_val},function(data){
					if(data === 'false') {
						alert(lang.name_exist);
						$('span[fieldname="'+s_name+'"][fieldid="'+s_id+'"]').text(s_value);
						return;
					}else {
						$this.attr("selectedid",new_val);
					}
				});
		...
		.val(selectedid)
			...
			$('<input type="text" id="date-editor">').css({border:'1px solid #ccc',width:'80px',height:'25px'}).datepicker().appendTo($(this).parent())
			.val(s_value)
			.blur(function(){
				var $this = $(this);
				setTimeout(function(){
					...
					$.get(url,{id:s_id,column:s_name,value:$this.val()},function(data){
						if(data === 'false') {
								alert(lang.name_exist);
								$('span[fieldname="'+s_name+'"][fieldid="'+s_id+'"]').text(s_value);
								return;
		...
		$('<input type="text">').css({border:'1px solid #ccc',width:'80%',height:'25px'})
		...
		if(event.keyCode == 13) {
			...
			$.get(url,{id:s_id,column:s_name,value:$(this).attr('value')},function(data){
				if(data === 'false') {
					alert(lang.name_exist);
					$('span[fieldname="'+s_name+'"][fieldid="'+s_id+'"]').text(s_value);
					return;
...
//给需要修改的图片添加异步修改行为
$('img[ectype="inline_edit"]').live("click",function(){
	var i_id    = $(this).attr('fieldid');
	var i_name  = $(this).attr('fieldname');
	var i_src   = $(this).attr('src');
	var i_val   = ($(this).attr('fieldvalue'))== 1 ? 0 : 1;
	$.get('main.php?app='+APP+'&act=ajax_col&is_ajax=1',{id:i_id,column:i_name,value:i_val},function(data){
	if(data.success){
		...
		}else if(i_src.indexOf('negative')>-1){alert("2");
