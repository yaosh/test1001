...
$(function(){
	//setTable();
	$("table.setting").hide();
	//$("table.setting:first").show();
	$("table.post").show();
	
	isShowReason();
	tongjiTr();
	banTr();
	$("li.setting-tab a").live('click' , function(data){
		$("li").removeClass('active');
		$(this).parents('li').addClass('active');
		type = $(this).attr("type");
		setTable();
	});
	$("li.setting-tab:first a").click();
	type = $("li.setting-tab.active a").attr("type");
...
function checkForm(){
	...
	if(type == "base"){
		...
		var mailGroup = $.trim($("#mailGroup").val());
		if(!mailHost){
		...
		if (!mailGroup) {
			cmf("您还没有填写群邮箱","mailGroup");
			return false;
		}
		pd = {"type":type , "host":mailHost , "port":mailPort , "user":mailUser , "pass":mailPass, "group":mailGroup};
	}else if (type == 'ban'){
